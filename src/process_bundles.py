...
def get_last_processed_bundle(chain_id: int) -> int:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT MAX(bundle_id) FROM Bundle WHERE chain_id = ?",
            (chain_id,)
        )
...
def get_last_bundle_end_block(bundle_id: int, chain_id: int) -> int:
    ...
    try:
        ...
        logger.error(f"Error getting block for bundle {bundle_id} on chain {chain_id}: {str(e)}")
        return 0
    finally:
        conn.close()


def get_spoke_bundle_events(chain_id: int, relayer_roots: List[str], start_block: int) -> List[Dict]:
    """
...
def process_chain_bundles(chain_id: int, hub_contract) -> None:
    ...
    try:
        ...
        bundle_events = get_spoke_bundle_events(chain_id, relayer_roots, last_bundle_end_block)
            ...
            for propose_event in propose_events:
                ...
                matching_spoke_event = next(
                    (e for e in bundle_events 
                     if e["args"]["relayerRefundRoot"] == propose_event["args"]["relayerRefundRoot"]),
                    None
                )

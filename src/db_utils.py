...
def insert_route(
    ...
    try:
        ...
        if existing_record:
            logger.debug(
                f"Route already exists with ID {existing_record[0]['route_id']}"
            )
            return None
...
def insert_token(token_address, chain_id, symbol, decimals):
    ...
    try:
        existing_record = execute_query(
            exists_query, (token_address, int(chain_id)), fetchall=True
        )
        if not existing_record:
            ...
            cursor = execute_query(
                insert_query,
                (token_address, int(chain_id), symbol, decimals),
                commit=True,
            )
